...
impl Document {
    ...
    pub fn replace_diagnostics(
        ...
        if unchanged_sources.is_empty() {
            if let Some(provider) = provider.as_ref() {
                self.diagnostics.retain(|d| &d.inner.provider != provider);
            } else {
            ...
            self.diagnostics.retain(|d| {
                if provider
                    .as_ref()
                    .is_some_and(|provider| provider != &d.inner.provider)
                {
                    return true;
                }
    ...
    pub fn clear_diagnostics_for_server(&mut self, id: LanguageServerId) {
        self.diagnostics
